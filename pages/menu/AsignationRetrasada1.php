...
if(!isset($_REQUEST['cargar'])) {
    ...
    $tpl = new TemplatePower("AsignationRetrasada1.tpl");
...
$tpl->gotoBlock("_ROOT");
$verificacion = $objControlCourse->getActivationRetrasada1();
$isActivated=false;
...
$periodoAsignacionR1=0; //parametro para buscar las ordenes de pago para ese periodo
$anioSis = 0;
        ...
        if($row['estado']==1)
        {
            ...
            switch($row['schoolyear'])
            {
                case PRIMERA_RETRASADA_DEL_PRIMER_SEMESTRE:
                    ...
                    $periodoAsignacionR1 = PRIMERA_RETRASADA_DEL_PRIMER_SEMESTRE;
                ...
                case PRIMERA_RETRASADA_DEL_SEGUNDO_SEMESTRE:
                    ...
                    $periodoAsignacionR1 = PRIMERA_RETRASADA_DEL_SEGUNDO_SEMESTRE;
...
$_SESSION["datosGenerales"]->periodo = $periodoAsignacionR1;
...
if($isActivated){
    ...
    $txtMensaje1='SISTEMA ACTIVO PARA ASIGNACIÓN DE PRIMERA RETRASADA';
}else
{
    $txtClase1='msg-danger-txt';
    $txtIcono1='fa fa-close fa-2x';
    $txtMensaje1='SISTEMA NO ACTIVO PARA ASIGNACIÓN DE PRIMERA RETRASADA';
}
...
else
{
    ...
    $result = $objControlCourse->getCursosRetrasada1($objuser->getId(), $anioEnviar, $periodoEnviar,$periodoActual,$anioSis,$objuser->getCareer(),DIAS_ANTES_DE_PAGO_RETRASADAS);//enviamos la consulta
    $resultOrden=$objControlCourse->searchOrdenR1($objuser->getId(), $anioSis, $periodoAsignacionR1); //realiza consulta para verificar si existe orden de pago y devuelve el codigo de la orden
    $resultPago=$objControlCourse->searchOrdenPagadaR1($objuser->getId(), $anioSis, $periodoAsignacionR1,$objuser->getCareer());
    ...
    if($result){
        if(!$resultOrden){ //Si no hay ordenes pendientes de pago
            ...
            foreach ($resultOrden as $orden) {
                ...
                {
                    $temporal = $objControlCourse->searchDetalleOrdenR1($orden['paymentorder']); //Crea bloque para boletas generadas
                    if($temporal){
