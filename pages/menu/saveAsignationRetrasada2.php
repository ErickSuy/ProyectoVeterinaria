...
$tpl = new TemplatePower("saveAsignationRetrasada2.tpl");
...
$idpaymenttype=0;
if ($periodoDeAsignacion==SEGUNDA_RETRASADA_DEL_PRIMER_SEMESTRE){
    $idpaymenttype=_TIPO_PAGO_SEGUNDA_RETRASADA;
}else if($periodoDeAsignacion==SEGUNDA_RETRASADA_DEL_SEGUNDO_SEMESTRE)
{
    $idpaymenttype=_TIPO_PAGO_SEGUNDA_RETRASADA2;
}
...
$_SESSION["montoGeneral"]=$monto; //Se asigna el monto total de los cursos asignados
...
if($_SESSION["contador"] ==0){
    ...
    $vectorClasesXML = array($posCont,
        ...
        'montoLaboratorio'   => 0,
        'curso'   => $rRow['idcourse'],
        'seccion'   => $objControlCourse->getSection($objuser->getId(),$rRow['idcourse']),
        'numHoras'   => 0,
        'marca'   => FALSE,
        'retUnica'   => FALSE
        );
    ...
    if($resultadoOb){
        $resultInsertOrden=$objControlCourse->insertOrdenRetrasada2($_SESSION['ORDEN_PAGO']['numeroOrden'], $monto, VERCA, VERWS, $idpaymenttype, $objuser->getCareer(), $objuser->getId(), VERIFIER, REQUESTYPE, $anioAsignacion, $periodoDeAsignacion, $_SESSION['ORDEN_PAGO']['identificador'], $_SESSION['ORDEN_PAGO']['rubropago']);
        if($resultInsertOrden)
        {
            $objControlCourse->insertDetalleRetrasada2($_SESSION['ORDEN_PAGO']['numeroOrden'], VALOR_SEGUNDA_RETRASADA, 0, $rRow['idcourse'], 0);
        }
