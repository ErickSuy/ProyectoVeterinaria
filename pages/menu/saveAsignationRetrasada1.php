...
$tpl = new TemplatePower("saveAsignationRetrasada1.tpl");
...
$idpaymenttype=0;
if ($periodoDeAsignacion==PRIMERA_RETRASADA_DEL_PRIMER_SEMESTRE){
    $idpaymenttype=_TIPO_PAGO_PRIMERA_RETRASADA;
}else if($periodoDeAsignacion==PRIMERA_RETRASADA_DEL_SEGUNDO_SEMESTRE)
{
    $idpaymenttype=_TIPO_PAGO_PRIMERA_RETRASADA2;
}
...
if($_SESSION["contador"] ==0){
    ...
    if($resultadoOb){
        $resultInsertOrden=$objControlCourse->insertOrdenRetrasada1($_SESSION['ORDEN_PAGO']['numeroOrden'], $monto, VERCA, VERWS, $idpaymenttype, $objuser->getCareer(), $objuser->getId(), VERIFIER, REQUESTYPE, $anioAsignacion, $periodoDeAsignacion, $_SESSION['ORDEN_PAGO']['identificador'], $_SESSION['ORDEN_PAGO']['rubropago']);
        if($resultInsertOrden)
        {
            $objControlCourse->insertDetalleRetrasada1($_SESSION['ORDEN_PAGO']['numeroOrden'], VALOR_PRIMERA_RETRASADA, 0, $rRow['idcourse'], 0);
        }
